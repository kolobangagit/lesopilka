...
function t228__init(recid) {
    ...
    rec.addEventListener('click', function (e) {
        var targetLink = e.target.closest('.t-menusub__target-link'); if (targetLink && window.isMobile) { if (targetLink.classList.contains('t-menusub__target-link_active')) { if (menuBlock) menuBlock.dispatchEvent(overflowEvent) } else { if (menuBlock) menuBlock.dispatchEvent(noOverflowEvent) } }
...
function t674_init(recid) {
    var rec = document.getElementById('rec' + recid); if (!rec) return; var backgroundImage = rec.querySelector('.t674__img-holder'); if (!backgroundImage) return; backgroundImage.onload = function () {
        var delay = 0; var allRecords = document.getElementById('allrecords'); if (allRecords.getAttribute('data-blocks-animationoff') !== 'yes') { delay = 600 }
        setTimeout(function () { document.body.classList.add('t674__body_with-bg') }, delay)
    }; var event = document.createEvent('HTMLEvents'); event.initEvent('load', !0, !1); backgroundImage.dispatchEvent(event)
}
function t576_init(recId) {
    var rec = document.getElementById('rec' + recId); if (!rec) return; var container = rec.querySelector('.t576'); if (!container) return; var items = rec.querySelectorAll('.t576__item'); var firstItem = items[0]; var lastItem = items[items.length - 1]; for (var i = 0; i < items.length; i++) { var item = items[i]; var circle = item.querySelector('.t576__circle'); var img = item.querySelector('.t576__img'); var imgHeight = 0; if (img) imgHeight = img.offsetHeight; if (circle) circle.style.top = (imgHeight + 15) + 'px' }
